...
#########################################
### najem addons
    coadaptation_procedure = True
    coadaptation_active = True
    coadaptation_criteria = 0.65
##########################################
	...
	#PATH TO COPY IN THE TERMINAL: cd /firmm/rt-fmri/META-BRAIN/pyDecNef-static/2.neurofeedback_training/1.realtime_fMRI_scripts'''
        ...
        #cls.raw_volumes_folder = cls.moduledir.parent / '3.fMRI_simulator_realdata/outputs' # To use with fMRI simulator  
        cls.raw_volumes_folder = Path('/firmm/20240903.Anna_plt2.Anna_plt2') # To use in a real experiment setting
        ...
        #cls.model_file = cls.resources_dir / 'models/tf_model.h5'
        cls.model_file = cls.resources_dir / 'models/sklearn_decoder'
        check_file(cls.model_file)

        # Region of interest mask path (as .nii to maximize load speed)
        cls.mask_file = cls.resources_dir / 'masks/example_func_deoblique_brainmask.nii'
        ...
        if cls.zscoring_procedure == 'to_model_session':
            cls.zscoring_mean =  cls.resources_dir / 'training_zscoring_data/zscoring_mean_array.npy' # Numpy array containing mean of model construction session data
            cls.zscoring_std = cls.resources_dir / 'training_zscoring_data/zscoring_std_array.npy'  # Numpy array containing standard deviation of model construction session data
